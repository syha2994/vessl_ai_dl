...
class AnalogGaugeInspector:
    ...
    def process_image(self, image_name):
        ...
        # 가장 큰 컨투어를 바늘로 간주
        needle_contour = max(contours, key=lambda cnt: cv2.arcLength(cnt, closed=True))
        ...
        if 0.15 < abs(dist1 - dist2) / dist1:
            ...
            dist_to_border_1 = distance_to_border(needle_point_1[0])
        ...
        if deg_mag < EPS:
            ...
            while True:
                if estimated_value > params.max_value:
                    if estimated_value - params.max_value >= params.tick_interval * 0.9:
                        estimated_value = self.params.min_value
                        break
                if estimated_value < params.min_value:
                    if params.min_value - estimated_value >= params.tick_interval:
                        estimated_value = self.params.max_value
                        break
                    else:
                        estimated_value = params.min_value
                        break
                break
