...
from sklearn.metrics import confusion_matrix, accuracy_score, f1_score, precision_recall_curve, roc_curve
...
def cal_img_roc(scores, gt_list):
    ...
    return fpr, tpr, thresholds, img_roc_auc
...
def cal_acc(scores, gt_list, best_threshold):
    img_scores = scores.reshape(scores.shape[0], -1).max(axis=1)
    gt_pred = (img_scores>best_threshold).astype(int)
    gt_pred_list = gt_pred.tolist()
    tn, fp, fn, tp = confusion_matrix(gt_list, gt_pred).ravel()

    # Accuracy 계산
    acc = accuracy_score(gt_list, gt_pred)

    # F1 score 계산
    f1 = f1_score(gt_list, gt_pred)

    fn_idx = []
    fp_idx = []

    for i in range(len(gt_list)):
        if gt_list[i] == 1 and gt_pred_list[i] == 0:
            fn_idx.append(i)
        elif gt_list[i]==0 and gt_pred_list[i] == 1:
            fp_idx.append(i)


    return tn, fp, fn, tp, acc, f1, fn_idx, fp_idx